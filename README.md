...
*Comparison between the guidance reference $\gamma_{ref}$ generated by the Sine Pitch Program and the vehicle's actual flight path angle. Successful tracking confirms the PID control loop correctly drives the vehicle to the target suborbital injection condition $\gamma = 0°$ at MECO (T+231s).*

![GNC Trajectory Tracking](results/GNC%20Trajectory%20Tracking.png)

### Figure 2: Actuator Telemetry

*TVC actuator deflection $\delta_c$ over the full flight duration. The Sine Pitch Program concentrates control demand in the early, low-speed phase and progressively reduces it as dynamic pressure increases, keeping the actuator within the $\pm 10°$ physical limit throughout the hypersonic phase.*

![Actuator Telemetry](results/Actuator%20Telemetry.png)

### Figure 3: Flight Profile

*Powered ascent trajectory over a spherical Earth. The profile captures the initial vertical pad clearance, followed by the actively steered pitch-over maneuver driven by the Sine Pitch Program. The trajectory flattens just below the Karman Line (100 km) as the vehicle accelerates horizontally, meeting the exact $\gamma = 0^\circ$ condition at MECO to establish the necessary velocity vector for the subsequent suborbital ballistic coast phase.*
